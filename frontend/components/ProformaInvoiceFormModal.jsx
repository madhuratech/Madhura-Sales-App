...
export default function ProformaInvoiceFormModal({
  open,
  editId,
  quotations,
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-6xl p-7 relative">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-xl font-bold text-gray-800">
            {editId ? "Edit Proforma Invoice" : "Create Proforma Invoice"}
          </h2>
          <X className="cursor-pointer text-gray-400 hover:text-red-500" onClick={resetForm} />
        </div>

        {/* Quick Fill from Quotation */}
        {!editId && quotations && quotations.length > 0 && (
          <div className="mb-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <label className="block text-sm font-semibold text-[#0088CC] mb-2">
              📋 Quick Fill from Quotation
            </label>
            <select
              onChange={e => handleSelectQuotation(e.target.value)}
              className="w-full border border-blue-300 rounded-lg px-3 py-2 outline-none text-sm bg-white focus:border-[#0088CC] focus:ring-1 focus:ring-[#0088CC]"
            >
              <option value="">Select a Quotation to auto-fill...</option>
              {quotations.map(q => (
                <option key={q.id} value={q.id}>
                  {q.customer_name} - {q.client_company || "N/A"} (QT-{String(q.id).slice(-4)})
                </option>
              ))}
            </select>
            <p className="text-xs text-gray-500 mt-1">
              Selecting a quotation will auto-fill all client and line item details
            </p>
          </div>
        )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={performaInvoice.invoice_date}
                    onChange={e => setPerformaInvoice({ ...performaInvoice, invoice_date: e.target.value })}
                    className="border border-gray-300 rounded-lg px-3 py-2 outline-none text-sm w-full focus:border-[#0088CC] focus:ring-1 focus:ring-[#0088CC]"
                    required
                  />
                </div>
